...
When the target project or changed files use Go, apply the `go-concurrency` skill:

- Prefer goroutines, channels, and `select`/`case` coordination for asynchronous work.
- Avoid introducing mutex locks when channel ownership can coordinate the state.
- Avoid introducing internal `context.Context` plumbing when one-shot feedback/done channels can express completion, failure, or abandonment.
- Send one-time feedback channels through channel messages when handlers need to report results, errors, completion, or cancellation state.
- Use non-blocking `select` checks to stop work when the caller no longer needs the task, then terminate the goroutine and close owned feedback channels.
- Add panic/recover handling at goroutine or handler boundaries and report failures through the feedback channel before closing it.
- Preserve project conventions and required standard-library/framework/external APIs; if `context.Context` or sync primitives are required at a boundary, use them and explain the exception.
