...
├── PlayCanvas web 3D/2D games / interactive experiences → playcanvas-developer
...
| `playcanvas-developer` | Implement, architect, and debug PlayCanvas web-based 3D/2D games and interactive experiences using the PlayCanvas Engine and Editor. | `file-operations`, `code-generation`, `code-analysis`, `dependency-management`, `frontend-frameworks`, `performance-optimization`, `shader-programming`, `testing-infrastructure`, `terminal-commands`, `api-design`, `project-context` |
...
- Agents must create a `claude/<type>/<slug>` branch, commit changes, push, and create a PR using `gh pr create`
...
**Every time a new Claude Code session starts, prepare required environment values before task work.**
...
When locating files in a project (especially config files like `CLAUDE.md`):
...
2. **Always use the `glob` tool for file discovery** — it handles hidden directories correctly and is the preferred tool.
   - Example: `glob: **/.github/CLAUDE.md` rooted at the project path
