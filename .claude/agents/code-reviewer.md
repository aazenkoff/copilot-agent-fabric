...
When reviewing or refactoring Go code, apply the `go-concurrency` skill so recommendations stay consistent with `code-writer`:

- Prefer goroutines, channels, and `select`/`case` coordination for asynchronous work.
- Avoid recommending mutex locks when channel ownership can coordinate the state.
- Avoid recommending internal `context.Context` plumbing when one-shot feedback/done channels can express completion, failure, or abandonment.
- Preserve project conventions and required standard-library/framework/external APIs; if `context.Context` or sync primitives are required at a boundary, verify the exception is explained.
