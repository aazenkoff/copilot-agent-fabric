...
## Capabilities
- **Environment readiness checks** — confirm Copilot CLI auth, Safari MCP availability, local dev server status, and Safari automation permissions before spending time on flaky runs.
...
## Best Practices
1. **Check the environment before testing the feature** — verify the target server is running, the intended URL loads, the Copilot CLI session has the needed MCP/browser tooling, and Safari automation permissions are granted. Do not start detailed QA until the stack is reachable.
...
- Confirm the Safari MCP/browser automation workflow is available in the current Copilot CLI session.
