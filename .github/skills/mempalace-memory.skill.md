...
└─ 5. Detect project context               (registry lookup + copilot-instructions.md)
```
...
5. **Detect project context** — check the project registry (via `project-context` skill or `mempalace_search(query="project registry", wing="tooling")`). Load the target project's `copilot-instructions.md` before writing any code.
...
- Information already in `copilot-instructions.md`
